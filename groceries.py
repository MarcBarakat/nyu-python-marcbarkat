...
for product in sorted(products,key = lambda k:k["name"]):
    print("+ " + product["name"] + " ($" + str('%.2f' % round(product["price"],2)) + ")" )
    if (product["department"] in departments.keys()):
        departments[product["department"]] = departments[product["department"]]+1
    else:
        departments[product["department"]] = 1
print("--------------")
print ("THERE ARE " + str(len(departments.keys())) + " DEPARTMENTS:")
